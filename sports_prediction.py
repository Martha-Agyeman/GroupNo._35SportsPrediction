...
numericVariables_imputed_df.isnull().sum().tolist() # checking to see if there are any null values still
...
categoricalVariables_imputed_df.isnull().sum().tolist() # checking to see if there are any null values still
...
X.drop(columns=['sofifa_id','categorical_No','categorical_Yes'],inplace= True)
...
from pandas.core.arrays import categorical
numeric_TestVariables = df2.select_dtypes(include=['int64','float64'])
categorical_TestVariables = df2.select_dtypes(include=['object'])
...
testData.drop(columns=['sofifa_id','categorical_No','categorical_Yes'],inplace= True)
...
pickle_filename = '/content/drive/My Drive/Colab Notebooks/XGBmodel_gs_model.pkl'
